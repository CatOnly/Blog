...
**由于浮点数的存储方式，浮点数表示不太精确，而是以很小的数来近似表示原数**，这种表示使浮点数是离散的会导致浮点数计算不太精确，例：0.1 + 0.2 != 0.3 
...
$$
Float_{max} = 1.11...1 \times 2^{254 - 127} = 3.402823e^{38}
$$
...
用以 10 为底的十进制浮点数替代基础类型中以 2 为底的二进制浮点数，常用**二到十进制编码**（称为 BCD 编码，常用的有NBCD 编码，8421 编码）序列来表示以下计数方法：
$$

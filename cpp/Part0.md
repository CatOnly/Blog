...
$$
\pm \infin = \pm 1.0 \times 2^{255 - 127} = \pm 2^{128}
$$
...
疑问：0.1 + 0.2 != 0.3 但是 0.1 + 0.2 输出结果是 0.3 ？
...
- 定点整数
  定点**无**符号整数：以补码形式存储，以 16 bit 存储范围 $[0, \space 2^{16} - 1]$
  定点**有**符号整数：高位第一位存储符号位，以补码形式存储，以 16 bit 存储范围 $[-2^{15}, \space 2^{15} - 1]$
- 定点小数
  需要确定该表示法的小数点位置（定点：又称为定标、Q值表示法）
  定点**无**符号小数：
  定点**有**符号小数：高位第一位存储符号位，
...
方法：通过乘 10 的指数倍将浮点数小数部分全部**四舍五入**转到正数部分，再进行整数计算，最后转为浮点数用来展示，例：0.1 + 0.2 = (1 + 2) / 10 = 3 / 10 = 0.3 

注意：不四舍五入则会因为乘 10 的指数倍后的浮点数精度不足带来误差，例：66.24 * 100 = 6623.999...91
